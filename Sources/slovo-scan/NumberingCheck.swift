...
/// Перевірка бази невідповідностей нумерації (`inconsistencies.sqlite3`) —
/// на справжньому файлі автора й справжніх модулях власника.
///
/// Перевіряємо три речі: база читається, наш запис дає той самий склад, і
/// перерахована адреса потрапляє в наявний вірш іншого перекладу.
func runNumberingCheck(modulesURL: URL) -> Int32 {
    ...
    // 1. Круговий прогін запису: пишемо свою копію, читаємо назад, звіряємо.
    ...
    // 2. Відомі місця Псалтиря: східний рахунок проти західного.
    print("\nпересчёт адресов (Псалтирь, книга 230):")
    let samples: [(Int, Int, String, String)] = [
        (22, 1, "ru", "ua"),   // Синодальний «Господь — Пастырь мой» = 23-й псалом
        (9, 22, "ru", "ua"),   // кінець 9-го псалма = початок 10-го
        (11, 2, "ru", "en"),
        (146, 1, "ru", "ua"),
        (23, 1, "ua", "ru"),
    ]
    // Рушій один на всю програму — той самий, що збирає слайд.
    ...
    // 3. Справжні модулі: перерахована адреса мусить існувати.
    //
    // Модуль призначення беремо не навмання, а за самою базою: у таблиці `modules`
    // записано скорочення (`BibleShortName`), і стандарт модуля відомий
    // тільки звідти. Модуль-джерело — той, якого в таблиці немає: в автора
    // перелічено тільки винятки, решта вважається східним рахунком.
    ...
    // Який у модуля рахунок — видно з самого Псалтиря: у східному рахунку
    // 10-й псалом короткий (7 віршів), у західному довгий (18). На це й
    // спираємося: таблицю `modules` в автора заповнено за мовою видання, і
    // трьом українським перекладам стандарт «ua» проставлено даремно — рахують
    // вони по-східному. Заради цього редактор і потрібен, але перевіряти
    // перерахунок треба на модулі, який справді рахує по-західному.
    ...
    // Для порівняння: скільки промахів дає той самий прогін без перерахунку.
    var raw = 0
    for chapter in sourceChapters {
        for verse in chapter.verses {
