...
///
/// Минусовка на служении играет ПОД слайдами: стих, куплет, картинка,
/// смена вкладки — а звук идёт. Проверяем ровно это на звуковом файле без
/// видео: открыли, заиграло, дальше делаем всё, что делает оператор, и
/// после каждого шага звук обязан играть, а файл — оставаться открытым.
extension Diagnostics {

    /// Короткий WAV с синусом: 44,1 кГц, моно, 16 бит. Своими руками, без
    /// кодеков — чтобы проверка не зависела ни от чего на машине.
    ...
    static func backingTrackSection(state: AppState) -> [Check] {
        ...
        defer {
            media.close()
            media.showStill(nil)
            // Пробные файлы в список плеера не должны попасть насовсем:
            // список переживает запуск, и после проверки в нём оставались
            // «минусовка.wav» и «slovo-проба-…», которых уже нет на диске.
            media.clearPlaylist()
        ...
        // 1. Стих в зал поверх звука.
        state.mode = .bible
        Signals.shared.send(.mode)
        state.showCurrent()
        settle(); expect("вірш у зал")

        // 2. Листание стиха в зал (стрелка/пульт).
        ...
        // 4. Смена вкладки на Песни и обратно на Медиа.
        state.mode = .songs
        Signals.shared.send(.mode)
        ...
        // 5. «Скрыть» показ — звук не трогает.
        state.isLive = false
        ...
        // Независимый проигрыватель: минусовка играет, пока в основном плеере
        // открывают и закрывают фильм.
