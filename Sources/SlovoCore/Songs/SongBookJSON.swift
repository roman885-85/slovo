...
///
/// Власник: «формат vbi та vbm — це формат VisioBible; для нас потрібен свій,
/// але так, щоб не поламати імпорт пісенників чи перекладів із VisioBible та
...
///    як і раніше) та ЕКСПОРТУ (планшет розбирає лише `.vbm`, VisioBible на
///    Windows — теж): `SongBookWriter` збирає його з тієї самої структури;
///  • поля заголовка VisioBible (`formatVersion`, `flags`, `charset`) їдуть
...
public enum SongBookJSON {
    ...
    /// VisioBible). Повертає, куди лягло.
    @discardableResult
